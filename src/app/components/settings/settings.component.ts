...
export class SettingsComponent implements OnInit, OnDestroy {
  ...
  public updatePasswordBtn: boolean = true;


  public IframeBox: boolean = false;

  public kycBox: boolean = true;

  public changeGeneralBtn: boolean = true;
  public enable2faDiv: boolean = false;
  public loginLoaderShow: boolean = false;

  public twoFaLoader: boolean = false;
  public authenticateApiLoader: boolean = false;


  public ToTpUri: any;
  public TwoFAFormattedKey: any;
  public disable2FA: boolean = false;

  public config: ToasterConfig =
    new ToasterConfig({ animation: 'flyRight' });

  public twoFaCode: number;
  public authBtn: boolean = true;
  public disable2faLoader: boolean = false;
  public state: any = "true";



  public kycLoader: boolean = false;

  public kycBtn: boolean = true;
  ...
  getkycstatus() {
    this._userService.getKycStatus(this.userObject.UserId).subscribe(a => {
  ...
  getSelectedNationality(event) {
    ...
    var obj = this.countries.find(x => x.alpha2Code == event);
  ...
  getSelectedCountry(event) {
    ...
    var obj = this.countries.find(x => x.name == event);
  ...
  submitKyc() {
    ...
    let IDTYPE = this.idType == 'INE' ? 'INE' : 'PASAPORTE';
    if (this.idType === 'INE' && (!this.frontPhotoFile || !this.backPhotoFile)) {

      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', `Please Upload ${this.idType} Front & ${this.idType} Back Image`):
      this.toasterService.pop('error', 'Error', `Favor de subir ${IDTYPE} frente y ${IDTYPE} Posterior imagen`);
      error = true;
    } else if ( this.idType === 'PASSPORT' && !this.frontPhotoFile) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', `Please Upload ${this.idType} Front Image`):
      this.toasterService.pop('error', 'Error', `Favor de subir ${IDTYPE} frente imagen`);
      error = true;
    } else if(!this.idType) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', 'Please Select ID Type'):
      this.toasterService.pop('error', 'Error', 'Por favor elige el tipo de Identificacion');
      error = true;
    }

    let BILLTYPE = this.billType == 'PHONE' ? 'Teléfono' : 'Electricidad';
    if ((this.billType === 'PHONE' || this.billType === 'ELECTRICITY') && !this.billPhotoFile) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', `Please Upload ${this.billType} Bill Image`):
      this.toasterService.pop('error', 'Error', `Favor de Subir el Comprobante de Domicilio`);
      error = true;
    } else if (!this.billType) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', 'Please Select Bill Type'):
      this.toasterService.pop('error', 'Error', 'Por favor elige el comprobante de domicilio');
      error = true;
    }

    if (this.phoneNumber == undefined) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', 'Phone Number Cannot Be Empty'):
      this.toasterService.pop('error', 'Error', 'Numero de telefono no puede estar vacio');
      error = true;
    }
    ...
    if (this.selectedCode == undefined) {
      ...
      this.toasterService.pop('error', 'Error', 'Codigo de Pais no puede estar vacio');
      error = true;
    }

    if (this.selectedCode && this.selectedCode.length < 2 || this.selectedCode.length > 4) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', 'Code Must Be Between 2 to 4 Digits'):
      this.toasterService.pop('error', 'Error', 'El codigo del pais debe tener entre 2 y 4 digitos');
      error = true;
    }

    if (this.phoneNumber && this.phoneNumber.length < 7 || this.phoneNumber.length > 10) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', 'Phone Must Be Between 7 to 10 Digits'):
      this.toasterService.pop('error', 'Error', 'El número telefónico debe ser de 7 y 10 dígitos');
      error = true;
    }
    ...
    if (error) {
      ...
      this._userService.doKyc(this.userObject.UserId, this.idType, this.frontPhotoFile, this.backPhotoFile, this.billType, this.billPhotoFile, this.phoneNumber, this.userObject.Name, this.selectedCode).subscribe(a => {
  ...
  checkLength() {
    ...
    var fieldLength = $("#twoFaCode").val().length;
  ...
  disable2FAFunction() {
    ...
    if (!this.validations.verifyNameInputs(this.twoFaCode)) {
      ...
      this.toasterService.pop('error', 'Error', "Favor de introducir el codigo 2FA");
      error = true;
    ...
    if (error) {
      ...
      this._userService.authenticate2Fa(this.userObject.Email, "false" , this.twoFaCode, this.userObject.TwoFAFormattedKey, this.userObject.ToTpUri, this.userObject.UserId).subscribe(a => {
  ...
  authenticate2Fa() {
    ...
    if (error) {
      ...
      this._userService.authenticate2Fa(this.userObject.Email, "true", this.twoFaCode, this.TwoFAFormattedKey, this.ToTpUri, this.userObject.UserId).subscribe(a => {
  ...
  enableDisable2FA(event) {
    ...
    if (event) {
      ...
      this._userService.enableDisable2FA(this.userObject.Email, this.userObject.UserId).subscribe(a => {

        if (a.code == 200) {
          this.twoFaLoader = false;
          var url = "https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl=200x200&chld=M%7C0&cht=qr&chl=";
  ...
  updateGeneralSettings() {
    ...
    if (!this.validations.verifyNameInputs(this.userObject.Name)) {
      ...
      this.toasterService.pop('error', 'Error', "nombre no pueden estar vacios");
      error = true;
    ...
    if(this.userObject.hasOwnProperty("WalletAddress")){
      ...
      if (this.userObject.WalletAddress == "" && this.userWalletAddress == "") {
        this.userObject.Language == 'en' ?
        this.toasterService.pop('error', 'Error', "ETH address can't be empty."):
        this.toasterService.pop('error', 'Error', "La Dirección ETH no puede estar vacía");
        error = true;

      } else if(this.userObject.WalletAddress != "" && this.userWalletAddress == ""){
        this.userObject.Language == 'en' ?
        this.toasterService.pop('error', 'Error', "ETH address can't be empty."):
        this.toasterService.pop('error', 'Error', "La Dirección ETH no puede estar vacía");
        error = true;
    ...
    if (error) {
      ...
      this._userService.changeGeneralSettings(this.userObject.UserId, this.userObject.Name, this.userWalletAddress).subscribe(a => {
        ...
        if (a.code == 200) {
          ...
          localStorage.setItem("userObject", JSON.stringify(this.userObject));
          this._sharedService.changeUserObject(JSON.parse(localStorage.getItem("userObject")));
        }
      }, err => {
  ...
  updatePassword() {
    ...
    if (!this.validations.verifyNameInputs(this.oldPassword)) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', "Old Password Cannot be Empty") :
      this.toasterService.pop('error', 'Error', "Contraseña anterior no puede estar vacio");
      error = true;
    ...
    if(this.password && !this.validations.verifyPasswordLength(this.password)) {
      error = true;
      this.userObject.Language == 'en' ?
    ...
    if (!this.validations.verifyNameInputs(this.cPassword)) {
      this.userObject.Language == 'en' ?
      this.toasterService.pop('error', 'Error', "Password and confirm password must match"):
      this.toasterService.pop('error', 'Error', "Ambas contraseñas deben coincidir");
      error = true;
    ...
    if (error) {
      ...
      this._userService.changePassword(this.oldPassword, this.password, this.userObject.UserId).subscribe(a => {
    ...
    if (input.files && input.files[0]) {
      if (input.files[0].type == 'image/png' || input.files[0].type == 'image/jpeg' || input.files[0].type == 'application/pdf') {
        if (input.files[0].size < 5242880) {
          var self = this;
            reader.onload = function (e) {
              if (type == 'front') {
                ...
                self.frontPhoto =self.frontFileType==='application/pdf' ? self.pdfUploaded : e.target.result;
                self.frontPhotoFile = input.files[0];
                
              } else if (type == 'back') {
                self.backFileType = input.files[0].type;
                self.backPhoto = self.backFileType==='application/pdf' ? self.pdfUploaded : e.target.result;
                self.backPhotoFile = input.files[0];
               
              } else if (type == 'bill') {
                self.billFileType = input.files[0].type;
                self.billPhoto =self.billFileType==='application/pdf' ? self.pdfUploaded : e.target.result;
          ...
          this.toasterService.clear();
          this.userObject.Language == 'en' ?
          this.toasterService.pop('error', "Error", "Max upload size is limited to 5 MB"):
          this.toasterService.pop('error', "Error", "El documento es muy pesado, favor de subir una imagen menor a 5mb");
        }

      } else {
        this.toasterService.clear();
        this.userObject.Language == 'en' ?
        this.toasterService.pop('error', "Error", "Only jpg, png, and pdf file supported"):
        this.toasterService.pop('error', "Error", "Solo archivos .jpg y .png ");
      }
  ...
  showKycDetails(user, kycDetails) {
    ...
    if (kycDetails.idType == 'INE') {
      ...
      {
        let file=kycDetails.backImage.split('?');
        let tempEx=file[0].split('.')
        this.backFileType=tempEx[5];
        this.backPhoto=this.backFileType=="pdf" ? this.pdfUploaded:kycDetails.backImage;
      }
    }
    if(kycDetails.frontImage){
      let file=kycDetails.frontImage.split('?');
      let tempEx=file[0].split('.')
      
      this.frontFileType=tempEx[5];
      
      this.frontPhoto=this.frontFileType=="pdf"? this.pdfUploaded:kycDetails.frontImage;
      
      }
      if(kycDetails.billImage)
      {
        let file=kycDetails.billImage.split('?');
        let tempEx=file[0].split('.')
       
        this.billFileType=tempEx[5];
        this.billPhoto=this.billFileType=="pdf"? this.pdfUploaded:kycDetails.billImage;
