...
export class SignUpComponent implements OnInit, OnDestroy {
  ...
  onNameInput(event: Event, name: string): void {
    ...
    this.SignUpObject[name] = input.value.replace(/^\s+|\s+$/g, ''); // Ensures no leading or trailing spaces
  }
  ...
  signUp() {
    ...
    if (!this.ValidationsClass.verifyNameInputs(this.SignUpObject.username)) {
      ...
      !this.ValidationsClass.verifyAffiliateCodeAndUsernmarLength(this.SignUpObject.username)
    ) {
      error = true;
      this.lang == 'en'
        ? this.toasterService.pop(
    ...
    if (this.SignUpObject.referralCode) {
      if (
        !this.ValidationsClass.verifyAffiliateCodeAndUsernmarLength(this.SignUpObject.referralCode)
      ) {
        error = true;
        this.lang == 'en'
          ? this.toasterService.pop(
