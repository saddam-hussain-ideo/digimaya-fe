...
export class SignUpComponent implements OnInit, OnDestroy {
  ...
  public emptyfullName: boolean = false;

  public epmtyLName: boolean = false;

  /*   public ethCheck: boolean = false; */

  public validPasswordCheck: boolean = false;

  public validCPasswordCheck: boolean = false;
  ...
  public emptyEthCheck: boolean = false;
  ...
  public resendEmailLoader: boolean = false;
  ...
  resendVerificationCode() {
    ...
    this._userService.resendEmailToUser(this.SignUpObject.email).subscribe(a => {
  ...
  actualSignUp() {
    ...
    this._userService.signUp(this.SignUpObject, this.captchaKey).subscribe(a => {
  ...
  signUp() {
    ...
    if (!this.ValidationsClass.verifyNameInputs(this.SignUpObject.email)) {
      ...
      this.toasterService.pop('error', 'Error', 'Correo electrónico requerido');
    ...
    if (!this.ValidationsClass.verifyNameInputs(this.SignUpObject.username)) {
      ...
      this.toasterService.pop('error', 'Error', 'Favor de ingresar un usuario valido');


    } else if(!this.ValidationsClass.verifyUserNameLength(this.SignUpObject.username)){
      error = true;
      this.lang == 'en' ?
      this.toasterService.pop('error', 'Error', 'Username should be between 3-20 characters'):
      this.toasterService.pop('error', 'Error', 'El nombre de usuario debe tener entre 3 y 20 caracteres');
    }else {
      if (!this.ValidationsClass.verifyUsername(this.SignUpObject.username)) {
        error = true;
        this.lang == 'en' ?
        this.toasterService.pop('error', 'Error', 'Username should be alphanumeric with only one underscore as a special character.') :
        this.toasterService.pop('error', 'Error', 'El nombre de usuario debe ser alfanumérico y solo un guion bajo como carácter especial');
      }
    }
    ...
    if (!this.ValidationsClass.verifyUserNameLength(this.SignUpObject.fullName)) {
      ...
      this.toasterService.pop('error', 'Error', 'Nombre Completo (8-50 caracteres)');
    ...
    if (!this.ValidationsClass.verifyNameInputs(this.SignUpObject.password)) {
      ...
      this.toasterService.pop('error', 'Error', 'La contraseña es requerida');
    }

    if(this.SignUpObject.password && !this.ValidationsClass.verifyPasswordLength(this.SignUpObject.password)){
      error = true;
      this.lang == 'en' ?
    ...
    if ($('#TermsCheck').is(':checked')) {
      ...
      error = true;
      this.lang == 'en' ?
      this.toasterService.pop('error', 'Error', 'Please accept terms and condition by clicking on check box terms & condition checkbox') :
      this.toasterService.pop('error', 'Error', 'Favor de aceptar los términos y condiciones dando clic en la casilla');
    }
    ...
    if (error) {
      ...
      console.log(this.SignUpObject.mobile);
      console.log(this.SignUpObject);
